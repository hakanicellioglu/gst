...
foreach ($products as $p) {
    $rows .= '<tr>' .
        '<td class="text-center py-1">'.htmlspecialchars($p['ral']).'</td>' .
        '<td class="text-center py-1">'.htmlspecialchars($p['glass']).'</td>' .
        '<td class="text-center py-1">'.htmlspecialchars($p['type']).'</td>' .
        '<td class="text-center py-1">'.$p['qty'].'</td>' .
        '<td class="text-center py-1">'.$p['width'].'</td>' .
        '<td class="text-center py-1">'.$p['height'].'</td>' .
        '<td class="text-center py-1">'.number_format($p['meterage'], 2).'</td>' .
        '<td class="text-center py-1">'.number_format($p['total_meterage'], 2).'</td>' .
        '<td class="text-center py-1">'.number_format($p['total_price'], 2).'</td>' .
    '</tr>';
}
...
<div class="col-md-6"><div class="row"><div class="col-4 fw-bold">Firma:</div><div class="col-8">'.htmlspecialchars($company).'</div></div><div class="row"><div class="col-4 fw-bold">Sayın:</div><div class="col-8">'.htmlspecialchars($contact).'</div></div><div class="row"><div class="col-4 fw-bold">Telefon:</div><div class="col-8">'.htmlspecialchars($contactPhone).'</div></div><div class="row"><div class="col-4 fw-bold">E-posta:</div><div class="col-8">'.htmlspecialchars($contactEmail).'</div></div></div>
<div class="col-md-6"><div class="row"><div class="col-4 fw-bold">Teklif No:</div><div class="col-8">'.$offerNumber.'</div></div><div class="row"><div class="col-4 fw-bold">Teklif Tarihi:</div><div class="col-8">'.$offerDate.'</div></div><div class="row"><div class="col-4 fw-bold">Hazırlayan:</div><div class="col-8">'.htmlspecialchars($preparedBy).'</div></div><div class="row"><div class="col-4 fw-bold">E-posta:</div><div class="col-8">siparis@alumann.com</div></div></div>
</div>
</header>
<table class="table table-bordered table-sm">
<thead class="table-dark"><tr><th class="text-center py-1">RAL Kodu</th><th class="text-center py-1">Cam Rengi</th><th class="text-center py-1">Sistem Tipi</th><th class="text-center py-1">Adet</th><th class="text-center py-1">En (mm)</th><th class="text-center py-1">Boy (mm)</th><th class="text-center py-1">Metraj (m)</th><th class="text-center py-1">Toplam Metraj (m)</th><th class="text-center py-1">Toplam Fiyat ₺</th></tr></thead>
<tbody>$rows</tbody>
<tfoot><tr><td colspan="8">Ara Toplam</td><td>'.number_format($subtotal,2).' ₺</td></tr><tr><td colspan="8">KDV %20</td><td>'.number_format($vat,2).' ₺</td></tr><tr><td colspan="8"><strong>Genel Toplam</strong></td><td><strong>'.number_format($grand,2).' ₺</strong></td></tr></tfoot>
</table>
<p><strong>Teslimat:</strong> '.$delivery.'</p>
<p><strong>Ödeme:</strong> '.$payment.'</p>
<p><strong>Teklif Geçerlilik:</strong> '.$validity.'</p>
<table class="table table-bordered table-sm bank-accounts mt-3">
<thead class="table-dark"><tr><th colspan="3" class="text-center py-1">Banka Bilgileri</th></tr><tr><th class="py-1 text-center">Banka Adı</th><th class="py-1 text-center">Firma Adı</th><th class="py-1 text-center">IBAN</th></tr></thead>
<tbody>$bankRows</tbody>
</table>
<table class="table table-bordered table-sm signature mt-4 w-100"><tr><td>Teklif Onayı</td><td>Müşteri Onayı</td></tr><tr><td class="py-4"></td><td class="py-4"></td></tr></table>
</div>
</body>
</html>';
